...
const Nav =({children}:Props)=>{
    ...
    return(
        ...
        <div id={styles.display_box}>
            ...
            <div id={styles.service_select}>
                <Tab site_link={"./service_select"} site_name={"トップ"}  setModal={setModal}/>
                <Tab site_link={"./template_select"} site_name={"テンプレ"}  setModal={setModal}/>
                <Tab site_link={"./scan"} site_name={"オリジナル"}  setModal={setModal}/>
                <Tab site_link={"./device_select"} site_name={"手書き"} setModal={setModal}/>
                <Tab site_link={"./site"} site_name={"公式サイト"}  setModal={setModal}/>
                <Tab site_link={"./help"} site_name={"ヘルプ"}  setModal={setModal}/>
            </div>
    ...
    const Tab = React.memo(function Memotab({site_link,site_name,setModal}:Tab_type){
    
        const[tab,setTab] = useRecoilState(tabState);
        
        return( 
            <Link href={site_link} as={site_link} passHref>
                <div id={styles.btn} className={`${tab === site_name && styles.select}`} onClick={()=>{setTab(site_name);setModal(false);}}>
                    {site_name}
                </div>
            </Link>
        )
